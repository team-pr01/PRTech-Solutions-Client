...
const TechStackCard = ({technologyName, icon} : {technologyName: string, icon: any}) => {
  return (
    <div className="relative bg-neutral-85/50 border border-primary-40 rounded-lg flex flex-col gap-0 lg:gap-2 items-center justify-center w-[70px] lg:w-[150px] h-[70px] lg:h-[140px]">
      ...
      <Image
        ...
        alt=""
      />
      <Image src={icon} alt={``} className="size-8 lg:size-12" />
      <h1 className="text-white text-[8px] lg:text-sm font-bold leading-5">{technologyName}</h1>
    </div>
  );
};
